 ...
 */
export function toApiError(status: number, message: string, errors?: Record<string, string[] | boolean>, rawForServerLog?: unknown): ApiError {
...
export async function parseApiErrorResponse(res: Response): Promise<ApiError> {
  ...
  try {
    ...
    return toApiError(res.status, "An error occurred while processing the request", undefined, undefined);
  }

  if (isApiErrorResponse(errorData)) {
    return toApiError(res.status, errorData.message, errorData.errors ?? undefined, errorData);
  }

  if (typeof errorData === "object" && errorData !== null && "message" in errorData && typeof (errorData as { message: unknown }).message === "string") {
    return toApiError(res.status, (errorData as { message: string }).message, undefined, errorData);
  }

  return toApiError(res.status, "An unexpected error occurred", undefined, errorData);
}
