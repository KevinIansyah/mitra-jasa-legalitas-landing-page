  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <div className="space-y-0.5">
        ...
        <p className="text-xs text-gray-500 dark:text-gray-400">{author.position ?? "-"}</p>
      </div>
    </div>
  );
}
...
export function RelatedPostsSidebar({ posts }: { posts: BlogCard[] }) {
  ...
  return (
    <div>
      <p className="text-xs font-bold uppercase tracking-widest text-gray-400 dark:text-gray-500 mb-4">Artikel Terkait</p>
          ...
          <li key={relatedPost.slug}>
            <Link href={`/blog/${relatedPost.slug}`} className="group flex gap-3 items-start">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-gray-800 dark:text-gray-100 leading-snug line-clamp-2 group-hover:text-brand-blue transition-colors">{relatedPost.title}</p>
...
export function SidebarCta() {
  return (
    <div className="rounded-2xl p-5 text-white space-y-3" style={{ backgroundColor: BRAND_BLUE }}>
      ...
      <p className="text-sm font-bold leading-snug">Butuh konsultasi legalitas bisnis?</p>
      <p className="text-xs text-white/70 leading-relaxed">Tim konsultan kami siap membantu proses legalitas bisnis Anda dari awal hingga selesai.</p>
      <Link href="/daftar" className="inline-flex items-center gap-1.5 text-xs font-semibold text-white underline underline-offset-2 hover:no-underline group">
        Konsultasi Gratis
