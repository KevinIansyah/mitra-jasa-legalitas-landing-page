      ...
      <Link href={`/blog/${post.slug}`} className="blog-card group flex flex-col rounded-2xl border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-card overflow-hidden h-full">
        <div className="relative h-[200px] w-full overflow-hidden shrink-0 bg-gray-100 dark:bg-white/5">
          {post.featured_image ? (
            ...
            <div className="absolute inset-0 flex items-center justify-center text-5xl bg-gray-100 dark:bg-white/5">📄</div>
          )}
...
export function BlogListClient({ initialListData, categories, initialUrlKey }: Props) {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-12 pb-16 lg:pb-20">
        <div className="flex items-center justify-between mb-8 flex-wrap gap-2">
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Menampilkan <span className="font-semibold text-gray-900 dark:text-white">{search.trim() ? filtered.length : blogs.length}</span> artikel
            ...
            {state.tag.length > 0 && (
              <>
                {" · "}
                <span className="font-semibold text-gray-900 dark:text-white">{state.tag.length} tag</span>
              </>
            )}
            {meta && !search.trim() && <span className="text-gray-400 dark:text-gray-500"> (total {meta.total})</span>}
          </p>
          {hasActiveFilters && (
            <button type="button" onClick={() => resetFilters()} className="text-xs font-semibold text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 underline transition-colors">
              Reset filter
            </button>
          )}
        </div>
        ...
        {error && (
          <div className="rounded-2xl border border-red-200 dark:border-red-900/40 bg-red-50/50 dark:bg-red-950/20 px-4 py-6 text-center mb-8">
            ...
            <button type="button" onClick={() => loadPage(1, state, false)} className="mt-3 text-sm font-semibold text-brand-blue hover:underline">
              Coba lagi
            </button>
          </div>
        )}
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">Tidak ada artikel yang cocok.</p>
            <button type="button" onClick={() => resetFilters()} className="text-sm font-semibold text-brand-blue hover:underline">
              Lihat semua artikel
            </button>
          </div>
        )}
        ...
        {loading && blogs.length > 0 && <p className="text-center text-sm text-gray-400 mt-8">Memuat…</p>}
      </div>
    </>
  );
}
