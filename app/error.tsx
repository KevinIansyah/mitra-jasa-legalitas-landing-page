...
export default function Error({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  ...
  const isLikelyNetworkFailure = /fetch failed|ECONNREFUSED|ENOTFOUND|ETIMEDOUT|ECONNRESET|network\s*error/i.test(msg);

  const title = isLikelyNetworkFailure ? "Tidak dapat terhubung" : "Terjadi kesalahan";
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 py-20 bg-gray-50 dark:bg-surface-subtle">
      ...
      <div className="max-w-md w-full rounded-2xl border border-gray-200 dark:border-white/10 bg-surface-card p-8 text-center shadow-sm">
        ...
        {isDev && (
          <pre className="mb-6 text-left text-xs text-destructive bg-destructive/10 border border-destructive/25 rounded-lg p-3 overflow-auto max-h-32 whitespace-pre-wrap wrap-break-word">
            {error.message}
            {error.digest ? `\n[digest: ${error.digest}]` : ""}
          </pre>
        )}
