...
import { fetchProposalsPage } from "@/lib/api/endpoints/proposals";
...
function ProposalCard({ row }: { row: Proposal }) {
  const statusMeta = getProposalStatusMeta(row.status);
  return (
    <Link
      href={`/portal/proposal/${row.id}`}
    ...
    >
      <div className="flex items-start justify-between gap-2">
        <div className="space-y-1.5">
          <p className="font-mono text-sm font-semibold text-gray-900 dark:text-white">{row.proposal_number}</p>
...
type Props = {
  initial: ProposalsListResult;
};

function normalizeMeta(initial: ProposalsListResult) {
...
export function ProposalsList({ initial }: Props) {
  const [rows, setRows] = useState<Proposal[]>(() => initial.data ?? []);
  ...
  const loadMore = useCallback(async () => {
    ...
    setError(null);
    try {
      const next: ProposalsListResult = await fetchProposalsPage(meta.current_page + 1);
      setRows((prev) => [...prev, ...(next.data ?? [])]);
      setMeta(normalizeMeta(next));
    } catch (e) {
      setError(e instanceof ApiError ? e.message : "Gagal memuat proposal.");
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      ...
      {rows.length === 0 ? (
        <PortalEmptyState
          ...
          description="Proposal yang diterbitkan untuk akun Anda akan tampil di sini."
        />
      ) : (
        <>
          <div className={cn("hidden md:block", PORTAL_DATA_TABLE_WRAP)}>
            <table className={PORTAL_DATA_TABLE}>
              <thead className={PORTAL_DATA_TABLE_HEAD}>
                <tr>
                  <th className="px-4 py-3 font-semibold text-gray-900 dark:text-white">No. proposal</th>
              ...
              <tbody className={PORTAL_DATA_TABLE_BODY}>
                {rows.map((row) => {
                  ...
                  <tr key={row.id} className="align-middle">
                    <td className="px-4 py-3 font-mono font-medium text-gray-900 dark:text-white">{row.proposal_number}</td>
                    <td className="max-w-[240px] truncate px-4 py-3 text-muted-foreground">{row.project_name}</td>
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <Button asChild variant="secondary" size="icon" className="rounded-md">
                            <Link href={`/portal/proposal/${row.id}`} aria-label={`Lihat detail proposal ${row.proposal_number}`}>
      ...
      {rows.length > 0 && (
        <p className="text-center text-xs text-muted-foreground">
          Menampilkan {rows.length} dari {meta.total} proposal
        </p>
      )}
    </div>
    </TooltipProvider>
