...
export function DetailHero({ heading, service, city, categoryColor }: DetailHeroProps) {
  return (
    <div className="bg-surface-page pt-16">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <nav className="flex items-center gap-1.5 text-xs font-semibold text-gray-400 dark:text-gray-500 mb-6 flex-wrap">
          ...
          <ChevronRight className="w-3.5 h-3.5 shrink-0" />
          <Link href={`/layanan/${service.slug}`} className="hover:text-gray-700 dark:hover:text-gray-200 transition-colors line-clamp-1">
            {service.name}
          </Link>
          <ChevronRight className="w-3.5 h-3.5 shrink-0" />
          <span className="text-gray-600 dark:text-gray-300 line-clamp-1">{city.name}</span>
        </nav>
        ...
        <h1 className="text-3xl lg:text-4xl xl:text-5xl font-extrabold text-gray-900 dark:text-white tracking-tight leading-tight mb-4 max-w-3xl">{heading}</h1>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        <div className="relative w-full h-[260px] sm:h-[340px] lg:h-[420px] rounded-t-3xl overflow-hidden bg-gray-100 dark:bg-white/5">
          {service.featured_image ? (
            <Image src={toR2ProxySrc(service.featured_image)} alt={heading} fill priority unoptimized className="object-cover" sizes="(max-width: 1200px) 100vw, 1152px" />
          ) : (
