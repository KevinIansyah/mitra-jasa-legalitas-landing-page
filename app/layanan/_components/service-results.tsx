    ...
    <div className="flex-1 min-w-0">
      ...
      {listError && (
        <div className="flex flex-col items-center justify-center py-16 gap-3 text-center rounded-2xl border border-red-200 dark:border-red-900/40 bg-red-50/50 dark:bg-red-950/20 px-4">
          ...
          <button type="button" onClick={onRetry} className="text-sm font-semibold text-brand-blue hover:underline">
            Coba lagi
          </button>
        </div>
      )}
      ...
      {!listError && !listLoading && displayed.length === 0 && (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">Tidak ada layanan yang cocok dengan filter Anda.</p>
          <button type="button" onClick={onResetFilters} className="text-sm font-semibold hover:underline" style={{ color: BRAND_BLUE }}>
            Reset filter
          </button>
        </div>
      )}
