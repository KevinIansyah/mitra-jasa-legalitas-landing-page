...
import { BRAND_BLUE, BRAND_ORANGE, sortOptions } from '@/lib/types/service';
          ...
          <div className="flex items-center gap-2 pb-2.5 overflow-x-auto scrollbar-none">
            {selectedCategories.map((slug) => (
              <button
                ...
                style={{ backgroundColor: BRAND_BLUE }}
              >
                <span className="sm:mb-0.5">{categoryLabel(slug)}</span>{' '}
                <X className="w-3 h-3" />
              </button>
            ))}
            {selectedPrices.map((id) => (
              ...
              >
                <span className="sm:mb-0.5">{priceLabelById(id)}</span>{' '}
                <X className="w-3 h-3" />
              </button>
            ))}
            <button
              ...
              className="shrink-0 text-xs font-semibold text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 underline ml-1 transition-colors"
            >
              Hapus semua
            </button>
          </div>
