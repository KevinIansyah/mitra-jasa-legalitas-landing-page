...
const perusahaan = [
  ...
  { label: 'Kisah Sukses', href: '/case-study' },
  { label: 'Testimoni', href: '/tentang/testimoni' },
  { label: 'Blog', href: '/blog' },
  { label: 'Karir', href: '/karir' },
];

const SOCIAL_ORDER: (keyof NavigationSocialMedia)[] = [
  'instagram',
  'facebook',
  'youtube',
  'linkedin',
...
export function Footer({ navigation }: FooterProps) {
  ...
  const tagline =
    company?.tagline ??
    'Solusi legalitas bisnis terpercaya di Indonesia. Kami mendampingi ribuan pengusaha dari pendirian badan usaha hingga perlindungan kekayaan intelektual.';
  ...
  return (
    <footer
      ...
      style={{ backgroundColor: 'oklch(0.13 0.02 260)' }}
    >
      {/* Blob */}
      <div
        aria-hidden
        ...
        style={{
          background:
            'radial-gradient(ellipse, oklch(0.3811 0.1315 260.22 / 0.12) 0%, transparent 70%)',
          filter: 'blur(40px)',
        }}
      />
              ...
              <div className="space-y-3">
                {contactRows.map((c) => (
                  ...
                  >
                    ...
                    />
                    <span className="text-xs text-white/50 group-hover:text-white/80 transition-colors leading-relaxed wrap-break-word max-w-[300px]">
                      {c.text}
                    </span>
                  </a>
                ))}
            ...
            <ul className="space-y-3">
              {serviceCategories.length === 0 ? (
                ...
                serviceCategories.map((cat) => (
                  <li key={cat.id}>
                    <Link
                      href={`/layanan?kategori=${encodeURIComponent(cat.slug)}`}
                      className="text-sm text-white/50 hover:text-white transition-colors"
                    >
                      {cat.name}
                    </Link>
