...
export function LeadFormBubble({ onSubmit, onSkip }: Props) {
  ...
  function validate(): boolean {
    ...
    else if (!/^(?:\+62|62|0)8[1-9]\d{6,11}$/.test(phone.replace(/[\s-]/g, '')))
      ...
      <AnimatePresence mode="wait">
        {done ? (
          ...
          <motion.div
            ...
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="bg-gray-100 dark:bg-white/10 rounded-2xl rounded-bl-sm px-4 py-3.5 max-w-[75%] w-full space-y-3"
          >
            {/* Header */}
            <div className="flex items-start justify-between gap-2">
              <p className="text-sm text-gray-800 dark:text-gray-100 leading-snug">
                Tinggalkan kontak Anda agar tim kami bisa follow-up 👇
              </p>
              <button
                type="button"
                onClick={onSkip}
      ...
      <div
        className={`flex items-center gap-2 bg-white dark:bg-white/10 rounded-lg px-2.5 py-2 border transition-colors ${
          error
            ...
            : 'border-gray-200 dark:border-white/15 focus-within:border-blue-400'
        }`}
      >
