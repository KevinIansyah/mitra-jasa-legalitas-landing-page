...
function DialogOverlay({ className, style, ...props }: React.ComponentProps<typeof DialogPrimitive.Overlay>) {
  return <DialogPrimitive.Overlay data-slot="dialog-overlay" className={cn("fixed inset-0 bg-black/50", className)} {...props} style={{ zIndex: 100, ...style }} />;
}
  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      <div className="fixed inset-0" style={{ zIndex: 50 }}>
        <DialogPrimitive.Overlay data-slot="dialog-overlay" className="fixed inset-0 z-0 bg-black/50" />
        <DialogPrimitive.Content
          data-slot="dialog-content"
          className={cn(
            "fixed top-1/2 left-1/2 z-10 flex min-h-0 w-[calc(100%-2rem)] max-w-lg -translate-x-1/2 -translate-y-1/2 flex-col gap-4 rounded-xl border border-border bg-card p-5 text-card-foreground opacity-100 shadow-lg outline-none isolate",
            className,
          )}
          {...props}
